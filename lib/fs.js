...
fs.readv = function(fd, buffer, position, callback) {
  if (length === 0) {
    return process.nextTick(function() {
      callback && callback(null, 0, buffer);
    });
  ...
  binding.readBuffers(fd, buffer, position, req);
};
...
fs.readBatchSync = function(fd, buffer, nr, offset, length, position) {
  return binding.readBatch(fd, buffer, nr, offset, length, position);
};
...
fs.writeBatchSync = function(fd, buffer, nr, offset, length, position) {
  return binding.writeBatch(fd, buffer, nr, offset, length, position);
};
